...
$(document).ready(function () {
  ...
  selectedDateRange();
  $(".custom-input-search").val(""); //Click on 7, 15, 30 day tab
  ...
  $('.select-days').click(function () {
    ...
    $('.custom-input-searchtable').val(''); // resetStatus();
...
function showOfflineQueriesTable() {
  ...
  var datatable = $('#offline-query-table').DataTable({
    ...
    'oLanguage': {
      ...
      oPaginate: {
        sFirst: offline_queries_js_var.first_page,
        // This is the link to the first page
        sPrevious: offline_queries_js_var.previous_page,
        // This is the link to the previous page
        sNext: offline_queries_js_var.next_page,
...
$(document).on('click', '#confirm-offline-action', function (event) {
  ...
  $.ajax({
    ...
    data: {
      ...
      'template_id': template_id ? template_id.split('_').join(' ') : template_id,
      'is_free_push': is_free_push
    },
    dataType: 'json',
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
  ...
  $.ajax({
    ...
    if (response.status === 200) {
      ...
      if (response.templates && response.templates.length > 0) {
        response.templates.forEach(function (templateId) {
...
$(document).on('click', '#offline-export', function (event) {
  event.preventDefault();
  var url = "/api/v1/offlineQueries/download?start_date=" + startDate + "&end_date=" + endDate + "&status=" + status;
  $.ajax({
    type: "GET",
    headers: {
