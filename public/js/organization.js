...
$(document).ready(function () {
  ...
  $(".custom-input-search").val(""); //edit form submit

  $(document).on('submit', '#edit-organization-form', function (event) {
    event.preventDefault();
    resetFormErrors(); // Set vars.
    ...
    if (form.find('[type=file]').length) {
      ...
      data = new FormData();
      contentType = false; // Append input to FormData object.
  ...
  $(document).on('submit', '#add-organization-form', function (event) {
    ...
    $.ajax({
      ...
      if (response.status === 200) {
        ...
        $('body').removeClass('overflow-hidden');
        showNotification('success', organization_js_var.added_successfully); // window.location = "/organization/index";
      }
    }).fail(function (response) {
      // Check for errors.
      if (response.status === 422) {
  ...
  $(document).on('click', '#edit', function (event) {
    organization_id = $(this).attr('data-id'); // Request.
  ...
  $(document).on('submit', '#add-organization-permission', function (event) {
    event.preventDefault(); // Set vars.

    var form = $(this);
    url = form.attr('action');
    var data = form.serialize();
    contentType = 'application/x-www-form-urlencoded; charset=UTF-8'; // Request.
...
function showOrganizationTable() {
  ...
  var datatable = $('#myTable').DataTable({
    ...
    oLanguage: {
      ...
      oPaginate: {
        sFirst: organization_js_var.first_page,
        // This is the link to the first page
        sPrevious: organization_js_var.previous_page,
        // This is the link to the previous page
        sNext: organization_js_var.next_page,
    ...
    columns: [{
      data: 'logo',
      name: 'logo',
      "class": 'prevent-overflow col-width-5',
      searchable: false
    }, {
      data: 'company_name',
      name: 'company_name',
...
function deleteOrganization(id) {
  ...
  $.ajax({
    ...
    if (response.status === 200) {
      ...
      $('body').removeClass('overflow-hidden');
      showOrganizationTable();
      showNotification('success', organization_js_var.delete_successfully); // window.location = "/organization/index";
    }
  }).fail(function (response) {
    var errors = $.parseJSON(response.responseText);
