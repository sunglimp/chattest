...
$(document).ready(function () {
  ...
  $(document).on('click', '#edit', function (event) {
    user_id = $(this).attr('data-id'); // Request.
  ...
  $(document).on('click', '#view', function (event) {
    ...
    $.ajax({
      ...
      if (response.status === true) {
        $("#detail_user_partial").html(response.html); // $('#view__popup').addClass('show');
  ...
  $(document).on('click', '#update_password_button', function (event) {
    ...
    var user_id = $("#user_id").val(); // Request.
...
function showUserTable() {
  ...
  var datatable = $('#myTable').DataTable({
    ...
    'oLanguage': {
      ...
      oPaginate: {
        sFirst: user_js_var.first_page,
        // This is the link to the first page
        sPrevious: user_js_var.previous_page,
        // This is the link to the previous page
        sNext: user_js_var.next_page,
  ...
  $('#select-organization').on('change', function (e) {
    datatable.draw(); // e.preventDefault();
  });
...
function showAddModal() {
  $.ajax({
    ...
    data: {
      organization_id: $('#select-organization').val()
    },
...
$(document).on('click', '#user-permission', function (event) {
  ...
  $.ajax({
    ...
    if (response.status === true) {
      ...
      $(".permission_error").addClass('hidden'); //openPopup('view');
    }
  }).fail(function (response) {
    // Check for errors.
    if (response.status === 422) {
...
$(document).on('submit', '#edituser_permission_form', function (event) {
  ...
  var form = $(this);
  url = form.attr('action'); // If found, prepare submission via FormData object.

  var input = form.serializeArray();
  var data = new FormData();
  var contentType = false; // Append input to FormData object.
