...
$(document).ready(function () {
  showCannedResponse(); //On click of add response.
  ...
  $("#add").click(function () {
    ...
    $("textarea").removeClass('warning-input');
  });
});
/**
 * Function to add canned response.
 * 
 * @returns
 */
      ...
      if (response.status == 422) {
        ...
        if (errors != undefined) {
          $.each(errors, function (field, message) {
            ...
            $("#" + field).parent().append('<span class="warning-text">*' + message + '</span>');
          });
        } else if (validation.message != undefined) {
          $("#shortcut").addClass('warning-input');
          $("#shortcut").parent().append('<span class="warning-text">*' + validation.message + '</span>');
        }
      } else {
        showNotification('warning', response.message);
      }
...
/**
 * Function to show datatable of canned responses.
 * 
 * @returns
 */


function showCannedResponse() {
  if ($.fn.DataTable.isDataTable('#canned-response-table')) {
    $('#canned-response-table').DataTable().destroy();
  }

  cannedResponseTable = $("#canned-response-table").DataTable(_defineProperty({
    "paging": true,
    "ordering": true,
    "bInfo": false,
    "serverSide": true,
    "fixedHeader": true,
    'oLanguage': {
      sEmptyTable: canned_js_var.no_data_available,
      sZeroRecords: canned_js_var.no_matching_records_found,
      oPaginate: {
        sFirst: canned_js_var.first_page,
        // This is the link to the first page
        sPrevious: canned_js_var.previous_page,
        // This is the link to the previous page
        sNext: canned_js_var.next_page,
        // This is the link to the next page
        sLast: canned_js_var.last_page // This is the link to the last page

      }
    },
    "order": [[2, "desc"]],
    "ajax": "/canned-response/ajax/show",
    "columnDefs": [{
      "targets": -1,
      "data": "img",
      "orderable": false,
      "render": function render(url, type, full) {
        if (full.can_update == 0) {
          return ' <span class="icon icon--accepted popup-btn disabled" id="edit" onClick="getEditData(\'' + full.cannedResponsId + '\')" disabled>' + '<i class="fas fa-pencil-alt"></i>' + '</span>' + '<span class="icon icon--rejected disabled" onClick="deleteCannedResponse(\'' + full.cannedResponsId + '\')">' + '<i class="fas fa-times"></i>' + '</span>	';
        } else {
          return ' <span class="icon icon--accepted popup-btn" id="edit" onClick="getEditData(\'' + full.cannedResponsId + '\')">' + '<span class="icon-hover-text">' + canned_js_var.edit + '</span>' + '<i class="fas fa-pencil-alt"></i>' + '</span>' + '<span class="icon icon--rejected" onClick="deleteCannedResponse(\'' + full.cannedResponsId + '\')">' + '<span class="icon-hover-text">' + canned_js_var["delete"] + '</span>' + '<i class="fas fa-times"></i>' + '</span>	';
        }
      }
    }, {
      className: " prevent-overflow col-width-10",
      "targets": [1]
    }],
    "columns": [{
      "data": "shortcut"
  ...
  }, "order", [])); // let scrollCount = 0;
  // $('#canned-response-table').on('header.dt,page.dt', function () {
  //     if (scrollCount++) {
  //         $('html, body').animate({
  //             scrollTop: $(".dataTables_wrapper").offset().top
  //         }, 'slow');
  //     }
  // });
  //enable searching for datatable

  $('#datatable-search').keyup(function () {
    cannedResponseTable.search($(this).val()).draw();
  }); //enable enteries per page for datatable

  $('#datatable-length').change(function () {
    cannedResponseTable.page.len($(this).val()).draw();
  });
}
/**
...
function deleteCannedResponse(cannedResponseId) {
  $('.overlay').show();
  $("#delete__popup").addClass('show');
  $("#confirm-message").html(messages.CANNED_RESPONSE_DELETE_CONFIRM);
  $("#delete__popup #cancel").html(messages.CANNED_RESPONSE_NO);
  $("#delete__popup #yes_confirmation").html(messages.CANNED_RESPONSE_YES);
  $("#yes_confirmation").off().click(function () {
    ...
    $.ajax({
      ...
      type: 'POST',
      data: postRequest,
      url: '/canned-response/ajax/delete',
      success: function success(response) {
        if (response.status === true) {
          showCannedResponse();
          showNotification('success', response.message);
...
/**
 * Function to edit cannedResponseId.
 * 
 * @param cannedResponseId
 * @returns
 */
...
function getEditData(cannedResponseId) {
  $('.overlay').show();
  $("#edit__popup").addClass('show');
  $(".warning-text").hide();
  $("input").removeClass('warning-input');
  $("textarea").removeClass('warning-input');
  populateEditDetails(cannedResponseId);
}
/**
 * Function to populate edit details.
 * 
 * @param cannedResponseId
 * @returns
 */
...
function populateEditDetails(cannedResponseId) {
  $('#shortcut').val('');
  $('#response').val('');
  var postRequest = {
    'cannedResponseId': cannedResponseId
  ...
  $.ajax({
    dataType: 'json',
    type: 'GET',
    url: '/canned-response/ajax/get-by-id',
    data: postRequest,
    success: function success(response) {
      $("#canned-response-id").val(cannedResponseId);

      if (response.data.shortcut != null) {
        var shortcut = response.data.shortcut;
      }
...
function editCannedResponse() {
  ...
  $("textarea").removeClass('warning-input');
  var postRequest = {
    'edit-shortcut': $("#edit-shortcut").val(),
    'edit-response': $("#edit-response").val(),
    'cannedResponseId': $("#canned-response-id").val()
  };
  $.ajax({
    dataType: 'json',
    type: 'POST',
    data: postRequest,
    url: '/canned-response/ajax/edit',
    success: function success(response) {
      if (response.status === true) {
    ...
    error: function error(response) {
      if (response.status == 422) {
        ...
        if (errors != undefined) {
          ...
          $("#edit-shortcut").addClass('warning-input');
          $("#edit-shortcut").parent().append('<span class="warning-text">*' + validation.message + '</span>');
        }
      } else {
        showNotification('warning', messages.SOMETHING_WENT_WRONG);
      }
