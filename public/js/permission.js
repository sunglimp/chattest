...
$(document).ready(function () {
    ...
    $.ajax({
      ...
      if (response.status === 422) {
        ...
        $.each(errors, function (field, message) {
          ...
          errorText.text(message); // var formGroup = $('[name=' + field + ']', form).closest('.popup__content--wrap');
          // formGroup.addClass('has-error').append('<p class="help-block" style="color:red">' + message + '</p>');
        });
      }
    });
  ...
  $(document).on('keydown', '.permissions-input', function (event) {
    ...
    if (event.keyCode === 13) {
      event.preventDefault();
      $("#".concat(id, "_button")).trigger('click');
    }
  });
...
function enableDisableSetting(element) {
  var settingElement = $(element).closest('tr').find('#setting-action').find('span'); //If permission is enabled for any role
...
$(document).on('click', '#missed_form_button', function (event) {
  ...
  var orgId = $('#organization_list').val(); // Request.
...
$(document).on('click', '#update-session-timeout-button', function (event) {
  ...
  var max_hours = $('#max-hours').val(); // Request.
...
function handleGroupServieType() {
  var val = $('#email__popup #provider_type').val();
  var encryptInput = document.getElementById("encryption");
...
function notificationSetting() {
  var notificationEvents = []; //console.log($('.notification-event:checkbox:checked').val());
